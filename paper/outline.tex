...
\begin{itemize}
  ...
  (cylinder, inactive/blue/white peg); backbone for 4.6--4.7. [TODO:
  training data, accuracy]
  \item \textbf{4.4 Path Length}: [TODO: computation method,
  kinematic tool-tip pose vs. vision-tracked cylinder]
  \item \textbf{4.5 Rate of Orientation Change}: Instrument-tip angular
  velocity, computed directly from PSM kinematics while the clock
  runs; follows Sharon et al.~\cite{sharon2021}, who found it separates
  expert from novice performance alongside task time and path length.
  [TODO: exact computation method, sampling rate]
  \item \textbf{4.6 Angular Displacement}: HSV segmentation on
  YOLOv8-detected cylinder crop; orientation estimate at placement.
  [TODO: color thresholds / angle derivation]
  \item \textbf{4.7 Object State Detection}: Correct/incorrect
  placement scoring from 4.6 against the lit target peg; separately,
  a 4-class state classifier (stationary/held/dropped/lost, out of the
  ECM's view) augmented with XGBoost~\cite{xgboost}. [TODO: classifier
  details, training data]
  \item \textbf{4.8 Ground Contact Detection}: ATI force sensor;
  displayed as a post-trial 2D graph (time vs.\ force in N), not
  streamed live. \emph{[Figure 4: force graph]}
  \item \textbf{4.9 3D Localization (Post-Hoc)}: SAM2-based~\cite{sam2}
  ...
  offline validation reference for 4.4/4.6, not a delivered real-time
