...
\subsection{Rate of Orientation Change}
\label{subsec:orientation_rate}

The rate of orientation change is computed directly from PSM
kinematics, the angular velocity of the instrument tip while the trial
clock runs. This measure follows Sharon et al.~\cite{sharon2021}, who
found it separated expert from novice performance on needle-driving
and suturing tasks, alongside classical measures such as task time and
path length.
% TODO: exact computation method, kinematic signal used, sampling rate.
...
dropped (off the board), and lost (out of the ECM's view).
% TODO: classifier details, training data, XGBoost augmentation.
% Cross-check against conductive-paint task board signal (subsec:task)
% and ground contact detection below.
...
x-axis and total force in newtons on the y-axis, showing how much force
...
position, used to check the real-time measures, path length and angular
displacement, against ground truth. It is not one of the
